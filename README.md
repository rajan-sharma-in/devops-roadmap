# DevOps Roadmap 2025

A hands-on plan to go from zero to productive DevOps engineer. This guide is arranged as a ladder of skills with curated resources, practice ideas, and context about when to invest in each area. Use it as a compass, not a rulebook—pick the next skill that helps you ship reliable software faster.

## Disclaimer
This roadmap highlights the landscape so you can make informed bets. It is not a checklist to blindly follow or a list of trendy tools. Learn the trade-offs, build intuition on why one tool fits better than another, and always evaluate the problem you are solving.

## Give a Star! ⭐
If this roadmap is part of your learning journey, please star the repo—it helps other learners discover it.

## Star History

---

## Table of Contents
- Workshop

## Workshop
- Launch Kubernetes local cluster
- Deploy Mautic
- GitOps workshop
- Ops workshop with GitLab self-host

as a DevOps you should know the linux well. so we will start with some basic commands 

## Additional Resources

### Tools
- **Work tracking**: Asana, Monday.com, Jira, Trello, Azure Boards
- **Source control**: Git, GitHub, GitLab, Bitbucket, Azure DevOps
- **CI/CD**: Jenkins, TeamCity, GitHub Actions, Travis CI, Bamboo, CircleCI, Azure Pipelines, Octopus Deploy, Harness, CodeShip
- **Source analysis**: SonarQube, Veracode
- **Artifacts**: JFrog Artifactory, Harbor, Docker Hub, npm, Yarn, NuGet
- **Configuration management**: Terraform, Ansible, Puppet, Chef
- **Container orchestration**: Docker, Kubernetes, Red Hat OpenShift
- **Monitoring**: Prometheus, Grafana, Splunk, Dynatrace, Kibana

### Books Worth Reading
- **The DevOps Handbook** — Kim, Debois, Willis, Humble
- **Accelerate** — Forsgren, Humble, Kim
- **Continuous Delivery** — Humble, Farley
- **Team Topologies** — Skelton, Pais
- **Effective DevOps** — Davis, Daniels
- **The Phoenix Project** — Kim, Behr, Spafford
- **Site Reliability Engineering** — Beyer, Jones, Petoff, Murphy
- **Fundamentals of DevOps and Software Delivery** — Brikman

---

---

## Wrap Up
Roadmaps evolve. If you see gaps, open an issue or a PR. Star the repo to get notified as the 2025 roadmap expands with labs and printable PDFs.

## Contribution Guide
1. Fork the repository.
2. Create a feature branch for your update.
3. Keep descriptions concise and link to reputable sources.
4. Open a pull request and describe why the change helps learners.

## License
[MIT License](LICENSE)

## Authors
- Rajan Sharma — Maintainer, DevOps Roadmap
